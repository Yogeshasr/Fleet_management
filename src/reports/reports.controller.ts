...
export class ReportsController {
  ...
  @ApiResponse({ status: 200, description: 'Fleet statistics retrieved successfully' })
  getFleetStatistics() {
  ...
  @ApiResponse({ status: 200, description: 'Trip reports retrieved successfully' })
  getTripReports() {
  ...
  @ApiResponse({ status: 200, description: 'Truck utilization reports retrieved successfully' })
  getTruckUtilization() {
  ...
  getFleetStatisticsDateRange(
    ...
    return this.reportsService.getFleetStatistics(new Date(startDate), new Date(endDate));
  }
  ...
  getTripReportsDateRange(
    ...
    return this.reportsService.getTripReports(new Date(startDate), new Date(endDate));
  }
  ...
  getDriverPerformanceDateRange(
    ...
    return this.reportsService.getDriverPerformance(new Date(startDate), new Date(endDate));
  }

  @Get('truck-utilization/:startDate/:endDate')
  @ApiOperation({ summary: 'Get truck utilization for date range' })
  @ApiResponse({ status: 200, description: 'Truck utilization retrieved successfully' })
  getTruckUtilizationDateRange(
    @Param('startDate') startDate: string,
    @Param('endDate') endDate: string
  ) {
    return this.reportsService.getTruckUtilization(new Date(startDate), new Date(endDate));
  }
  ...
  @ApiResponse({ status: 200, description: 'Driver rankings retrieved successfully' })
  getDriverRankings() {
  ...
  @ApiResponse({ status: 200, description: 'Route optimization insights retrieved successfully' })
  ...
  @ApiResponse({ status: 200, description: 'Fuel consumption analysis retrieved successfully' })
  getFuelConsumption() {
