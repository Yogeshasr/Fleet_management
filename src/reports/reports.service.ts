...
export class ReportsService {
  ...
  async getFleetStatistics(
    ...
    if (dateFilter) {
      tripsQuery.where("trip.startDate BETWEEN :startDate AND :endDate", {
        startDate,
        endDate,
      });
    }
    ...
    const revenueQuery = this.tripRepository
      .createQueryBuilder("trip")
      .select("SUM(trip.revenue)", "totalRevenue")
      .addSelect("SUM(trip.distance)", "totalDistance")
      .addSelect(
        "SUM(trip.fuelCost + trip.maintenanceCost + trip.otherExpenses)",
        "totalExpenses"
      );

    if (dateFilter) {
      revenueQuery.where("trip.startDate BETWEEN :startDate AND :endDate", {
        startDate,
        endDate,
      });
    }
  ...
  async getTripReports(
    ...
    if (startDate && endDate) {
      query.where("trip.startDate BETWEEN :startDate AND :endDate", {
        startDate,
        endDate,
      });
    }
    ...
    return trips.map((trip) => ({
      ...
      profit:
        trip.revenue -
        (trip.fuelCost + trip.maintenanceCost + trip.otherExpenses),
      status: trip.status,
    }));
  }
  ...
  async getDriverPerformance(
    ...
    if (startDate && endDate) {
      query.andWhere("trip.startDate BETWEEN :startDate AND :endDate", {
        startDate,
        endDate,
      });
    }
    ...
    return results.map((result) => ({
      ...
      efficiency:
        parseInt(result.completedTrips) > 0
          ? (parseInt(result.completedTrips) / parseInt(result.totalTrips)) *
            100
          : 0,
    }));
  }

  async getTruckUtilization(
    startDate?: Date,
    endDate?: Date
    ...
    const query = this.truckRepository
      ...
      .groupBy("truck.id")
      .addGroupBy("truck.licensePlate")
      .addGroupBy("truck.model");
    ...
    return results.map((result) => {
      ...
      return {
        ...
        maintenanceCost,
        utilizationRate:
          totalTrips > 0 ? Math.min((totalTrips / 30) * 100, 100) : 0, // Assuming 30 trips per month is 100% utilization
        profitability:
          totalRevenue > 0
  ...
  async getDashboardMetrics(): Promise<any> {
    const [fleetStats, tripReports, driverPerformance, truckUtilization] =
      await Promise.all([
    ...
    const weeklyRevenue = recentTrips.reduce(
      ...
      kpis: {
        fleetUtilization:
          totalActiveVehicles > 0
            ? (fleetStats.activeTrucks / fleetStats.totalTrucks) * 100
            : 0,
        driverUtilization:
          totalActiveDrivers > 0
            ? (fleetStats.activeDrivers / fleetStats.totalDrivers) * 100
            : 0,
        completionRate:
          fleetStats.totalTrips > 0
            ? (fleetStats.completedTrips / fleetStats.totalTrips) * 100
            : 0,
        averageDistance:
          fleetStats.totalTrips > 0
            ? fleetStats.totalDistance / fleetStats.totalTrips
            : 0,
      },
    };
  }
  ...
  async getFinancialSummary(): Promise<any> {
    ...
    const lastMonthStats = await this.getFleetStatistics(
      lastMonth,
      lastMonthEnd
    );
    ...
    const revenueGrowth =
      lastMonthStats.totalRevenue > 0
        ? ((fleetStats.totalRevenue - lastMonthStats.totalRevenue) /
      ...
      growth: {
        ...
        expenseGrowth:
          lastMonthStats.totalExpenses > 0
            ? ((fleetStats.totalExpenses - lastMonthStats.totalExpenses) /
  ...
  async getOperationalEfficiency(): Promise<any> {
    ...
    const fuelEfficiency =
      completedTrips.length > 0
        ? completedTrips.reduce(
            (sum, trip) => sum + trip.distance / (trip.fuelCost / 1.5),
            0
          ) / completedTrips.length // Assuming $1.5 per liter
        : 0;
    ...
    return {
      ...
      utilization: {
        fleetUtilization:
          trucks.length > 0
            ? (trucks.filter((t) => t.status === TruckStatus.AVAILABLE).length /
                trucks.length) *
              100
            : 0,
        driverUtilization:
          drivers.length > 0
            ? (drivers.filter((d) => d.status === DriverStatus.ACTIVE).length /
                drivers.length) *
              100
            : 0,
      },
  ...
  async getMaintenanceSchedule(): Promise<any> {
    ...
    const maintenanceData = trucks.map((truck) => {
      const totalMaintenanceCost = truck.trips.reduce(
        (sum, trip) => sum + trip.maintenanceCost,
        0
      );
      const totalDistance = truck.trips.reduce(
        (sum, trip) => sum + trip.distance,
        0
      );
      const avgMaintenancePerKm =
        totalDistance > 0 ? totalMaintenanceCost / totalDistance : 0;
    ...
    return {
      ...
      summary: {
        totalMaintenanceCost: maintenanceData.reduce(
          (sum, truck) => sum + truck.totalMaintenanceCost,
          0
        ),
        avgCostPerVehicle:
          maintenanceData.length > 0
            ...
            : 0,
        upcomingMaintenance: maintenanceData.filter(
          (truck) => truck.maintenanceStatus === "High Cost"
        ).length,
      },
    };
  }
    ...
    const rankings = driverPerformance
      .map((driver) => ({
        ...
        score:
          ...
          (driver.totalRevenue / 1000) * 0.3 +
          driver.averageRating * 20 * 0.3,
      }))
    ...
    return {
      ...
      categories: {
        byRevenue: rankings
          .sort((a, b) => b.totalRevenue - a.totalRevenue)
          .slice(0, 5),
        byEfficiency: rankings
          .sort((a, b) => b.efficiency - a.efficiency)
          .slice(0, 5),
        byTripsCompleted: rankings
  ...
  async getRouteOptimization(): Promise<any> {
    ...
    const routes = Object.values(routeAnalysis).map((route: any) => ({
      ...
      revenuePerKm:
        route.totalDistance > 0 ? route.totalRevenue / route.totalDistance : 0,
    }));

    return {
      routes: routes.sort((a, b) => b.revenuePerKm - a.revenuePerKm),
      insights: {
        mostProfitableRoute: routes.reduce(
          (max, route) => (route.revenuePerKm > max.revenuePerKm ? route : max),
          routes[0]
        ),
        mostFrequentRoute: routes.reduce(
          (max, route) => (route.totalTrips > max.totalTrips ? route : max),
          routes[0]
        ),
        longestRoute: routes.reduce(
          (max, route) => (route.avgDistance > max.avgDistance ? route : max),
          routes[0]
        ),
      },
    };
  ...
  async getFuelConsumption(): Promise<any> {
    ...
    const fuelAnalysis = trips.reduce(
      (acc, trip) => {
        const estimatedFuelConsumption = trip.fuelCost / 1.5; // Assuming $1.5 per liter
        const fuelEfficiency =
          trip.distance > 0 ? trip.distance / estimatedFuelConsumption : 0;
    ...
    return {
      summary: {
        ...
        totalFuelConsumed:
          Math.round(fuelAnalysis.totalFuelConsumed * 100) / 100,
        avgFuelEfficiency: Math.round(avgFuelEfficiency * 100) / 100,
        costPerKm:
          fuelAnalysis.totalDistance > 0
            ? fuelAnalysis.totalFuelCost / fuelAnalysis.totalDistance
            : 0,
      },
      trends: {
      ...
      vehicles: fuelAnalysis.trips.reduce((acc, trip) => {
        if (!acc[trip.truck]) {
          acc[trip.truck] = {
            truck: trip.truck,
            totalFuelCost: 0,
