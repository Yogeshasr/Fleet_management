...
export class TripsService {
  ...
  async findAll(filters?: { status?: string; driverId?: number; truckId?: number }): Promise<Trip[]> {
    ...
    if (filters?.driverId) {
      query.andWhere('trip.driverId = :driverId', { driverId: filters.driverId });
    }
  ...
  async update(id: number, updateTripDto: UpdateTripDto): Promise<Trip> {
    ...
    if (updateTripDto.status && !this.isValidStatusTransition(trip.status, updateTripDto.status)) {
      throw new BadRequestException(`Invalid status transition from ${trip.status} to ${updateTripDto.status}`);
    }

    // If completing or cancelling trip, free up the truck
    if (updateTripDto.status === TripStatus.COMPLETED || updateTripDto.status === TripStatus.CANCELLED) {
  ...
  async getFilteredTrips(filters: any) {
    ...
    if (filters.startDate) {
      query.andWhere('trip.startDate >= :startDate', { startDate: filters.startDate });
    }
  ...
  async startTrip(id: number) {
    ...
    if (trip.status !== TripStatus.PLANNED) {
      throw new BadRequestException('Trip can only be started if it is in PLANNED status');
    }
  ...
  async completeTrip(id: number) {
    ...
    if (trip.status !== TripStatus.IN_PROGRESS) {
      throw new BadRequestException('Trip can only be completed if it is in IN_PROGRESS status');
    }
